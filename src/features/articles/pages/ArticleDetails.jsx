...
const ArticleDetails = () => {
  ...
  function highlightWord(i) {
    ...
    if (currentHighlightedElementRef.current) {
      currentHighlightedElementRef.current.classList.remove(
  ...
  // unique article read tracking using fingerprint
  useEffect(() => {
    if (!articleId) return;
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        // console.log("RPC params:", articleId, fingerprint);
  ...
  // fetch article read count using useQuery
  const { data: readCount } = useQuery({
    queryKey: ["article-read-count", articleId],
    queryFn: async () => {
      const { count, error } = await supabase
        .from("unique_article_reads")
      ...
      await worker
        ...
        .then((pdf) => {
          ...
          for (let i = 1; i <= totalPages; i++) {
            ...
            pdf.text("The Fountainhead", pageWidth / 2, 12, {
              align: "center",
            });
          ...
          <div className={`containers ${styles.articleContainer}`}>
              ...
              >
                ...
                >
                  <div style={{ transform: "translateY(2px)" }}>
                    <i
                      style={{ fontSize: "20px" }}
                      className="fi fi-bs-cloud-download"
                    ></i>
                  </div>
                  &nbsp;&nbsp;
            ...
            <div className={`${styles.articleHead}`}>
              <h6 style={{ color: "grey", fontFamily: fontFamily }}>
                {language === "en"
                  ? article.event_title_en
                  : article.event_title_bn}{" "}
              </h6>
              ...
              <div style={{ textAlign: "center" }}>
                ...
                {!audioLoading && !audioUrl && (
                  <div style={{ fontSize: "14px", color: "gray" }}>
                    {audioError
                      ? "Audio unavailable"
                      : "No audio provided for this article."}
                  </div>
                )}
              </div>
