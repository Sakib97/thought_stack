...
const ArticleFeature = () => {
  ...
  const fetchFeaturedArticles = async () => {
    ...
    try {
      ...
      const formattedData = featuredData.map((item, index) => ({
        ...
        title: item.articles_secure?.title_en || "Untitled",
        author: item.articles_secure?.author_name || "Unknown Author",
    ...
    try {
      ...
      const { data: articles, error } = await supabase
        ...
        .not(
          "id",
          "in",
          `(${featuredIds.length > 0 ? featuredIds.join(",") : -1})`,
        )
  ...
  const handleAddArticle = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("featured_articles").insert({
  ...
  const handleRemoveArticle = async (articleId) => {
    ...
    try {
      ...
      const articleToDelete = data.find(
        ...
        >
          <SortableContext items={data.map((item) => item.key)}>
            <List
              ...
              dataSource={[
                ...data,
                ...(data.length < 5 ? [{ isAddButton: true }] : []),
              ]}
              renderItem={(item) => {
                if (item.isAddButton) {
                  return (
        ...
        {editingArticle && (
          <div style={{ marginTop: "20px" }}>
            ...
            <div style={{ color: "#666", fontSize: "13px" }}>
              Note: Dates cannot be set before today. End date must be after
              start date.
            </div>
          </div>
        )}
